...
**`src/api/hetzner.ts`** — `HetznerClient` wraps the Hetzner Cloud REST API v1. Uses `paginateList()` to follow cursor-based pagination automatically. Token validation calls `/servers?per_page=1` (not the root endpoint).
...
- Only Cloud API (multi-project) credentials are managed; Robot API and Storage Box support has been removed.
